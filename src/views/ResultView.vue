<template>
	<main ref="resultRoot" class="result-view" aria-labelledby="result-title" data-poster-page>
		...
		<div class="result-content">
			...
			<div class="result-controls">
				<div class="result-actions">
					<button class="result-button result-button--light" type="button" :disabled="isPosterRendering" @click="generatePoster">
						{{ isPosterRendering ? '正在生成基因海报…' : posterSnapshot ? '长按页面图片保存' : '生成我的基因海报' }}
					</button>
					<button class="result-button result-button--primary" type="button" @click="sharePoster">分享给朋友</button>
				</div>
		...
		<Transition name="share-guide">
			<div v-if="shareGuideVisible" class="share-guide" role="dialog" aria-modal="true" aria-label="分享指引" @click="shareGuideVisible = false">
				<div class="share-guide__content">
					<img class="share-guide__arrow" src="/assets/share-guide-arrow.png" alt="" aria-hidden="true" />
...
import { createRandomResultIndex, getResultProfile, normalizeResultIdentity, normalizeResultYear } from '../data/resultProfiles'
...
const getResultOptionIndex = (value) => {
	...
	return typeof queryValue === 'string' && /^[0-2]$/.test(queryValue) ? Number(queryValue) : createRandomResultIndex()
}
...
const waitForPosterAssets = async () => {
	...
	await Promise.all([document.fonts?.ready ?? Promise.resolve(), document.fonts?.load?.('700 16px "Resource Han Rounded CN"', posterFontText) ?? Promise.resolve(), ...imagePromises])
}
...
onMounted(() => {
	...
	if (resolvedResultRoute.fullPath !== route.fullPath) {
		void router.replace({ name: 'Result', query: stableResultQuery }).then(prepareWeChatShare)
	} else {
	...
	animationContext = gsap.context(() => {
		...
		entranceTimeline
			...
			.from(
				'.result-card__accent, .result-card__corner, .result-card__eyebrow',
				{
					autoAlpha: 0,
					x: -12,
					duration: 0.38,
			...
			.from(
				'.result-card__lead, .result-card__tag, .result-card__tag-line',
				{
					autoAlpha: 0,
					y: 12,
					duration: 0.42,
			...
			.from(
				'.result-card__divider, .result-card__quote, .result-card blockquote',
				{
					autoAlpha: 0,
					y: 10,
					duration: 0.46,
