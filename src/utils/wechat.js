...
const updateWeChatShareData = (method, data) =>
...
export const configureWeChatShare = async ({ title, description, link, imageUrl }) => {
	...
	if (!response.ok) {
		const details = [`HTTP ${response.status}`, config.wechatErrorCode ? `WeChat ${config.wechatErrorCode}` : '', config.outboundIp ? `outbound IP ${config.outboundIp}` : ''].filter(Boolean).join(', ')
	...
	await Promise.all([updateWeChatShareData('updateAppMessageShareData', appMessageData), updateWeChatShareData('updateTimelineShareData', timelineData)])
