...
- Timeline event content uses the current staged GSAP reveal. Overflow scrolling starts after the reveal plus a `0.6s` pause and moves at `30px/s`; keep this value unless the user asks otherwise.
...
- Timeline event dots, pulse rings, dates, cards, item copy, and images use a staged GSAP reveal. If content exceeds the panel, automatic movement starts after the full reveal plus 0.6s and travels at 30px/s. The panel remains vertically touch-scrollable, and any pointer, touch, wheel, or keyboard interaction stops automatic movement for the active year.
...
- `src/views/ResultView.vue`: result copy and year art come entirely from `getResultProfile()`. The 350 × 553px card stays at one consistent scale across device heights so the live page and generated full-page image never jump in size; short screens scroll vertically to reach controls, which remain interactive above the generated image.
...
- Historical note: the animation-free baseline below was superseded by the verified 2026-08-13 implementation. Keep its routing and first-frame safety constraints, but use the newer checkpoint as the source of truth for current Timeline animation and automatic scrolling.
...
- At the 2026-08-12 stability checkpoint, Timeline entrance/reveal animation was temporarily absent and Swiper remained a normal 1050ms horizontal slide. ScrollTrigger, Swiper Creative/flip-book effects, and the earlier speculative animation stack had been removed. The later 2026-08-13 active-slide implementation below is now authoritative.
...
- The safety direction established then still applies: keep animation active-slide-only, scoped with `gsap.context`, clean it up on slide changes/unmount, use transform/opacity, honor `prefers-reduced-motion`, and avoid ScrollTrigger, filters, layout-property animation, or animating every off-screen year.
...
## Timeline animation and automatic-scroll checkpoint (2026-08-13)

- `src/views/TimelineView.vue` now has an active-slide-only GSAP sequence. It grows the real `.timeline-line`, moves and fades `.timeline-line-head` during the initial line reveal, pops `.event-dot`, expands `.event-dot-pulse`, brings the date from the left, brings the card from the right/bottom, staggers card items, and settles active-year images. Later event groups reveal once as they enter the visible panel. The implementation is scoped with `gsap.context`, cancelled on slide changes/unmount, and skipped for `prefers-reduced-motion`.
- Do not restore the former scroll-handler behavior that created a new easing tween for the line or line head on every scroll event. It caused overlapping target-chasing and visible jitter. After the initial line reveal, the line is set to its complete length; scrolling only triggers a one-time reveal for newly visible event groups.
- Automatic overflow movement starts 600ms after the initial sequence and currently runs at exactly `30px/s` (`elapsedSeconds * 30`). It intentionally animates `.timeline-track` with a GPU-backed `y` transform instead of repeatedly writing fractional `scrollTop`; the user confirmed this track-transform version was the stable/non-jittery version in WeChat. Do not switch it back to per-frame fractional `scrollTop` without a verified replacement.
- Pointer, touch, wheel, or keyboard input immediately cancels automatic movement for the current year. Cancellation transfers the accumulated automatic position into the panel's native `scrollTop`, clears the track transform, and leaves manual vertical scrolling available. Changing years resets cancellation and rebuilds the active slide only.
- Timeline images continue to mount only for the active year. Do not animate or decode all 18 years on the first frame, add ScrollTrigger, or combine Timeline GPU work with the full-screen WeChat route transition. Preserve the complete `Home → Quiz → Timeline` WeChat regression check.
- The title highlight remains the user-tuned 70 × 21px SVG. `.year-heading__title img` is horizontally centered over its nested text span with `left: 50%` and `transform: translateX(-50%)`, while retaining `bottom: -2px` and the current layer order.
- Same-month timeline entries remain grouped into one card with multiple `event.items`, and every item can independently include its own image. Timeline assets in `public/assets/timeline/` are compressed WebP files. Known unresolved data references are `/assets/timeline/2014-11.webp` and `/assets/timeline/2014-12.webp`; no matching source images were available when the data was organized.
- The 2026-08-13 saved state passed `git diff --check` and `pnpm build`. The only build messages were the existing VConsole dependency warnings about `eval`.
