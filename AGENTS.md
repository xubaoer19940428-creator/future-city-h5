...
- The entire `/quiz` layout intentionally contains no `position: absolute`. `.quiz-flow` is a vertical Flex container: the 52px `.top-nav` is the first normal-flow row and `.quiz-track` fills the remaining height. The navigation uses a three-column Grid, so `.top-nav__title` is centered without positioning. `.intro-title` begins immediately after the navigation with no extra top gap. Intro copy and swipe hint are laid out by `.intro-content`; the hint is pushed down with `margin-top: auto`. Slide backgrounds, form tint, and form content share the same CSS Grid area instead of using positioned layers. The form heading also uses Grid to overlap its decorative highlight without affecting layout. Do not reintroduce `top`, `left`, `bottom`, or absolute positioning to these Quiz content blocks.
...
- Quiz uses a 52px normal-flow top navigation followed immediately by the remaining slide area. Each slide is a one-cell CSS Grid: background, optional tint, and content occupy the same grid area. Intro content is a vertical Flex layout; the swipe hint uses `margin-top: auto`. Form content is scrollable and the `.profile-form` owns bottom placement through Flex flow. There are intentionally zero `position: absolute` declarations in this file.
