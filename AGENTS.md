...
- Global UI font: `public/fonts/ResourceHanRoundedCN-Bold.woff2` via `src/styles/main.css`; the page and Result poster fetch use the same `v=20260811-2` cachebuster so WeChat/CDN clients do not retain the former subset.
...
- Timeline's `谱` and the Result trait characters `韧`, `押`, `弹`, `幕`, `监`, and `更` are confirmed present in the regenerated cmap. `.result-card__traits` now explicitly uses Resource Han Rounded CN at 700, and poster generation waits for all dynamic Result strings before capture, so live text and the embedded poster font use the same glyphs and weight.
...
- Result poster generation now uses `domToPng` from `modern-screenshot`, imported in the Result route chunk. After the slow Result GSAP entrance timeline completes, `ResultView.vue` captures the 370 × 553px `.result-card-capture` composition at 2× scale and overlays it with a real PNG `<img>`. The wrapper contains the 350 × 553px card plus the character's deliberate 20px right overflow; navigation, mascots outside the card, share controls, and “再测一次” are not part of the poster. `html2canvas` has been removed from `package.json` and is not the active Result capture implementation.
...
- Poster capture must use `posterCapture.offsetWidth/offsetHeight`, not `getBoundingClientRect()`. The Result stage scales to `0.9` or `0.82` on short phones, so the bounding rectangle reports transformed dimensions while `modern-screenshot` restores the clone to its natural layout; forcing those transformed dimensions caused the old crop and web/poster mismatch.
- The live `.result-stage__canvas`, `.result-card-capture`, and `.result-card-snapshot` share a 370 × 553px coordinate system. The canvas has `margin-left: 10px`, which preserves the original centered 350px card edge while leaving the extra 20px on the right for the character. Keep `.result-character` at `right: -20px` and 196 × 320px; the user explicitly rejected moving it to `right: 0`.
...
- `src/styles/main.css` explicitly sets both `-webkit-text-size-adjust: 100%` and `text-size-adjust: 100%` on `html` to stop iOS/Android text autosizing from changing layout geometry. The Result card and the `domToPng` clone repeat this normalization for poster parity.
...
- The poster wrapper fixes two separate bugs: capturing natural `offsetWidth/offsetHeight` avoids short-screen scale cropping, while using the same 370px live/snapshot parent prevents the generated PNG from appearing about 20px left of the webpage. The snapshot must not receive a separate rounded `border-radius`, because that would clip the character overflow already represented inside the PNG.
...
- Real browser screenshot tooling was unavailable for the final device comparison. In the next thread, first hard-refresh the affected iOS/Android device, wait for automatic poster generation (or tap retry), and compare the live card with the generated image. If a mismatch remains, inspect only this shared 370 × 553px composition before changing typography or the character position.
...
- `src/styles/main.css`: declares the compressed rounded WOFF2 globally, uses `html { font-size: 4.102564vw; }` as the no-JS fallback, normalizes platform text sizing to 100%, and keeps the app viewport fixed at full width/height.
...
- `src/views/ResultView.vue`: stable result query, collision-safe left text column, exact profile rendering, 370 × 553px natural-size capture wrapper, automatic 2× poster snapshot aligned with the live composition, preserved `right: -20px` character overflow, PNG QR asset, long-press save image, WeChat share guide, Web Share/clipboard fallback, and dynamic JSSDK setup.
